...
/**
 * The purpose of this class is to pay out a certain amount to each player besides the current
 * player, subtract the total amount paid out from the current player, and provide
 * a relevant String for a user-facing popup.
 * @author wyattfocht
 */
public class ChairManOfBoard extends Action {
   ...
   * @param resourcesPath
   */
  public ChairManOfBoard(int firstInt, int secondInt, List<Player> players,
  ...
  public boolean performAction() {
    if (playerCanPay(getFirstInt() * getOtherPlayersSize())) {
      editOtherPlayersWealth(getFirstInt(), 1);
      editPlayerWealth(getFirstInt() * getOtherPlayersSize(), -1);
    }
