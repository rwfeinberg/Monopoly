...
/**
 * The purpose of this class is to send a user straight to jail and provide
...
public class GoToJail extends Action {
   ...
   * @param resourcesPath
   */
  public GoToJail(int firstInt, int secondInt, List<Player> players,
  ...
  public boolean performAction() {
    ...
    int initialBalance = p.getWealth();
    movePlayerToFirstInstanceOf("JailTile");
    if (p.getWealth() != initialBalance) {
      p.editWealth(-1 * Math.abs(p.getWealth() - initialBalance));
    }
    p.setSuspended(getFirstInt());
